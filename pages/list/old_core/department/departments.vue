  ...
  <client-only>
    ...
    >
          ...
          <th v-for="item in headers" :key="item.name">
            ...
            <v-autocomplete
              v-if="
                item.filterable &&
                item.type === 'relation' &&
                item.value == 'department_parent.name'
              "
              v-model="item.dataFilter"
              :items="listParentDepartments"
              :label="item.text"
              item-text="name"
              item-value="id"
              return-object
      ...
      <template #top>
        ...
        <v-toolbar flat>
          <v-toolbar-title class="blue--text">Phòng ban</v-toolbar-title>
...
const deleteDepartmentGraphl = gql`
  mutation deletedepartments($id: Int) {
    delete_departments(where: { id: { _eq: $id } }) {
      affected_rows
    }
  }
`
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      // {
      //   text: 'Cấp trên',
      //   value: 'department_parent.name',
      //   type: 'relation',
      //   width: '160px',
      //   queryWhere: 'department_parent: {name: {_like: "%',
      //   dataFilter: '',
      //   filterable: true,
      //   class: 'blue lighten-2 font-weight-medium text-h4 white--text',
      // },
      ...
      // { text: 'Loại phòng bệnh', value: 'clinic_type_id' , class: 'blue lighten-2 font-weight-medium text-h4 white--text',},
      {
        text: 'Tạm ngưng',
        value: 'is_pause',
        type: 'boolean',
  ...
  apollo: {
    ...
    departments: {
      query() {
        ...
        const queryl = gql`query MyQuery {
                    departments(order_by: {${this.stringSort}},${this.filterWhere},limit:${this.limit} , offset: ${this.offset},) {
                      clinic_type_id
                      code
                      costcenter_id
                      department_type_id
        ...
        if (
          ...
          (this.isFilter && this.departments.length === 0)
        ) {
          this.loading = false
        }
        return queryl
    ...
    listParentDepartments: {
      query: gql(`query MyQuery {
                    departments(order_by: {updated_at: desc}, where: {internal_hospital_id: {_eq: ${localStorage.getItem(
                      'hospital'
                    )}}}) {
                      id
                      name
      ...
      result({ data }) {
        ...
        this.listParentDepartments = data.departments
      },
    },
    //   query: gql(`query MyQuery {
    //                 departments(order_by: {created_at: desc},${this.filterWhere}) {
    //                   clinic_type_id
    //                   code
    //                   costcenter_id
    //                   department_type_id
  ...
  methods: {
    ...
    add() {
      this.$router.push({
        path: `/list/old_core/department/department_edit`,
        query: { id: 0 },
      })
    },
    editItem(item) {
    ...
    refreshQuery() {
      this.loading = true
      this.$apollo.queries.departments.refresh()
    ...
    deleteItemConfirm() {
      this.idDepartment = this.departments[this.editedIndex].id
      this.departments.splice(this.editedIndex, 1)
      this.$apollo.mutate({
        mutation: deleteDepartmentGraphl,
        variables: {
          id: this.idDepartment,
        },
        update: (store, { data: { delete_departments } }) => {
          if (delete_departments.affected_rows) {
            console.log('xóa phòng ban')
            // eslint-disable-next-line
            this.textSnackbar = 'Xóa phòng ban thành công'
            this.snackbar = true
            this.refreshQuery()
          } else {
            this.textSnackbar = 'Xóa phòng ban thất bại'
            this.snackbar = true
          }
        },
      })
