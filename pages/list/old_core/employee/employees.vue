  ...
  <client-only>
    ...
    >
      ...
      <template #top>
        ...
        <v-toolbar flat>
          <v-toolbar-title class="blue--text">Nhân viên</v-toolbar-title>
...
export default {
  data: () => ({
    ...
    ethnic_groups: [],
    careers: [],
    filterWhere: `where: { type: { _eq: "NV" }, internal_hospital_id: {_eq: ${localStorage.getItem(
      'hospital'
    )}} }`,
    headers: [
      {
  ...
  // employees(where: {position: {type: {_eq: 0}}}) {
  apollo: {
    totalData: {
      query() {
        return gql(`query getTotalemployees {
            totalData: employees_aggregate(where: { type: { _eq: "NV" } }) {
                aggregate {
                    count
                }
            }
  ...
  methods: {
    ...
    add() {
      this.$router.push({
        path: `/list/old_core/employee/employee_edit`,
        query: { id: 0 },
      })
    },
    getTotalData() {
    ...
    searchAll() {
      ...
      if (boolCheckFilter) {
        this.filterWhere =
          this.filterWhere +
          `],type: { _eq: "NV" }, internal_hospital_id: {_eq: ${localStorage.getItem(
            'hospital'
          )}}}`
        this.isFilter = true
        // this.limit = eventData.limit
    ...
    refreshDatable() {
      this.isFilter = false
      this.filterWhere = `where: { type: { _eq: "NV" }, internal_hospital_id: {_eq: ${localStorage.getItem(
        'hospital'
      )}} }`
      this.$apollo.queries.employees.refresh()
    },
      ...
      this.$apollo.mutate({
        ...
        update: (store, { data: { delete_employees } }) => {
          if (delete_employees.affected_rows) {
            ...
            this.textSnackbar = 'Đã xóa nhân viên thành công'
            this.snackbar = true
            console.log(delete_employees.affected_rows)
          } else {
            this.textSnackbar = 'Xóa nhân viên thất bại'
            this.snackbar = true
